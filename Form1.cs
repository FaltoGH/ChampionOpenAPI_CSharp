...
using ChampionOpenAPI_CSharp.Properties;
...
#pragma warning disable CS0414 // The field 'Form1.g_bSetReal' is assigned but its value is never used
        bool g_bSetReal = false;        //실시간 여부
#pragma warning restore CS0414 // The field 'Form1.g_bSetReal' is assigned but its value is never used
#pragma warning restore IDE0052 // Remove unread private members
        ...
        public int ToInt(String sValue)
        {
            ...
            bool bSuccess = int.TryParse(sValue, out nRtn);
            if (bSuccess == false)
                return 0;
            else
                return nRtn;
        ...
        // String -> long 변환
        public long ToLong(String sValue)
        {
            long nRtn = 0;
            if (sValue.Trim().Length == 0)
                return 0;

            bool bSuccess = long.TryParse(sValue, out nRtn);
            if (bSuccess == false)
                return 0;
            else
                return nRtn;
        ...
        public double ToDouble(String sValue)
        {
            ...
            bool bSuccess = double.TryParse(sValue, out fRtn);
            if (bSuccess == false)
                return 0.0f;
            else
                return fRtn;
        ...
        // 프로그램 핸들 찾기(버전처리)
        void RunVersionCheckProcess(string file)
        {
            P("RunVersionCheckProcess("+file+')');
        ...
        private void Btn_VerChk_Click(object sender, EventArgs e)
        {
            ...
            if (regkey != null)
            {
                ...
                {
                    MessageBox.Show("OpenApi의 위치를 찾지 못했습니다.");
                    return;
                }
            }
            else
            {
                MessageBox.Show("프로그램의 위치를 찾지 못했습니다.");
                return;
            }
        ...
        private void Btn_Login_Click(object sender, EventArgs e)
        {
            ...
            string strPwd = TB_Pwd.Text;           // 로그인 비밀번호
            string strCretPwd = TB_CretPwd.Text;   // 인증 비밀번호
            string strPartnerCode = ComCode.Text;       // 제휴사 코드

            if (g_nVersionCheck == 0)
            {
                MessageBox.Show("로그인을 하려면 버전체크가 필요합니다.");
                return;
            }

            if (strID.Length == 0 || strPwd.Length == 0)
            {
                MessageBox.Show("로그인 입력값을 확인 바랍니다.");
                return;
            }

            if (strCretPwd.Length == 0)
            {
                DialogResult MsgRtn = MessageBox.Show("시세전용으로 로그인 하시겠습니까?\n인증서 비밀번호를 입력하시려면 취소를 누르십시오.", "확인", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
                if (MsgRtn == DialogResult.Cancel)
                {
                    return;
                }
            }
            ...
            else
            {
                ...
                MessageBox.Show(g_sMsg.Trim(), "로그인 실패");
                return;
            }
        }
